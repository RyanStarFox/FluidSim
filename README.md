...
c0..c8                 3x3 仿射速度矩阵（APIC 用，PolyPIC 可扩展）
...
| `g2p_flip()` | 网格 → 粒子（3D PIC-FLIP 混合，清空 c0..c8）|

### 参数规格

| 参数 | 值 |
|------|-----|
| 网格分辨率 | 64 x 80 x 64（3D）|
| 格子尺寸 | DX = 1/80 = 0.0125 |
| 粒子/格 | 8（2x2x2）|
...
output/
  flip/
    ratio_097/
      dam_break/
        frames/        # 帧图 PNG（300 张，720x720 透视 3D 渲染）
...
```bash
# 单次自检
/usr/bin/python3 plot_energy.py single output/flip/ratio_097/dam_break/energy.csv -o check.png
...
# 全量报告
/usr/bin/python3 plot_energy.py all --flip output/flip/ratio_097/ -o report/
```
